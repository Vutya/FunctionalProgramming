...
-- ДОПОЛНИТЕЛЬНО СО СЛЕДУЮЩЕЙ ЛЕКЦИИ
--
-- Двоичные деревья
-- В узлах значения, от каждого родителя максимум два потомка
-- data Tree a = EmptyTree | Node a (Tree a) (Tree a)
-- ещё бывает Leaf a | Node (Tree a) (Tree a), т.е. значения только в листьях
-- 
-- Что можно сделать с деревом?
-- 1) обобщить функцию map - treeMap
-- treeMap :: (a -> b) -> Tree a -> Tree b
-- Например, увеличить на единичку каждое значение в дереве
-- // подумать, как список, map и тип Maybe могу взаимодействовать
--
-- 2) обощить функцию fold 
-- foldTree :: b -> (a -> b -> b -> b) -> Tree a -> b
-- b - EmptyTree, тогда b - ответ, второй аргумент -TreeNode
-- по сути идём от листьев к корню
